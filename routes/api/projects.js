...
const PAYMENT_SECRET_KEY = require("../../constants");
  ...
  try {
    ...
    sales.insert({ ...saleDetails }).then(() => {
      if (saleDetails.customerId) {
        customers.findOne({ id: saleDetails.customerId }).then((doc) => {
          customers.update(
            { id: saleDetails.customerId },
            {
              $set: {
                bill: doc.bill - saleDetails.total,
              },
            }
          );
        });
...
router.post("/addCustomer", (req, res) => {
  ...
  try {
    customers.insert({ ...customerDetails }).then(() => {
      db.close();
      res.json({ success: true, message: "Customer Created", result: [] });
    });
  } catch (err) {
    res.json({ success: false, message: err.message });
  }
...
//Get Customers #mongodb
router.get("/getCustomers", (req, res) => {
  try {
    customers.find({}).then((doc) => {
      res.json({
        success: true,
    ...
    res.json({ success: false, message: "Failed", result: [] });
  }
});
